...
//
// `via` says every way in the edge exists for, and some of them matched on
// nothing — a default backend, a rule with neither host nor path. Reading the
// words put "default backend" where a consumer was promised a way in it could
// tell from another, which is worse than saying nothing: a name nothing can be
// told apart by is not a smaller answer, it is a wrong one.
