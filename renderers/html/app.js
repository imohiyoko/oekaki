...
(() => {
  ...
  function applyFolds() {
    ...
    for (const f of foldRecords) {
      ...
      if (agreed((m) => (m.claim ? m.claim.origin + '\u0000' + (m.claim.author || '') : ''))) {
        box.claim = members[0].claim;
      }
      const groups = {};
  ...
  function buildRoot() {
    ...
    for (const [i, e] of allEdges().entries()) {
      ...
      if (at) {
        at.infra.references++;
        // A copy, because `graph` is what the input said. Writing the count
        // onto the edge itself would edit the document the page carries, and
        // the next render would count the counts.
        at.infra.edge = {
          ...at.infra.edge,
          attrs: {...(at.infra.edge.attrs || {}), references: at.infra.references},
        };
        continue;
      }
      const drawnEdge = {
        id: 'e' + i, sources: [from], targets: [to],
        infra: {kind: 'edge', edge: e, references: 1},
      };
