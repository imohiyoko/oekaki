...
(() => {
  ...
  function layoutDocument() {
    ...
    return JSON.stringify({kind: 'oekaki.layout', version: '0.2',
      nodes: [...positions.values()].map((n) => {
        const size = sizes.get(n.id);
        return size ? {...n, ...(size.width ? {width: size.width} : {}),
          ...(size.height ? {height: size.height} : {})} : n;
  ...
  const layoutPost = document.body.dataset.layoutPost;
  const overlayPost = document.body.dataset.overlayPost;
  ...
  layoutExport.addEventListener('click', async () => {
    if (!layoutPost) { saveDocument(layoutDocument(), 'layout.json'); return; }
    ...
    try {
      const said = [await post(layoutPost, layoutDocument())];
      if (pending.length > 0 && overlayPost) {
        said.push(await post(overlayPost, overlayDocument()));
      }
      flash(said.join(', '));
  ...
  /* ---- mode ------------------------------------------------------------
     Read mode hides the export controls rather than merely refusing the
     gestures. A button that offers to export nothing is the part that made
     the old page ambiguous: a plain click on a box counted as a placement and
     put "Export layout" on screen, so the page looked as though something had
     been authored when nothing had. */
  ...
  function syncTools() {
    ...
    layoutExport.hidden = !editing
      || (positions.size === 0 && sizes.size === 0 && edgeAnchors.size === 0
        && lineShape === 'curved' && !(claims && layoutPost));
  }
