...
(() => {
  ...
  function applyFolds() {
    ...
    for (const f of foldsHere()) {
      ...
      // What every member agrees on and nothing else, which is the rule the
      // same box is built by in Go. Taking the first member's answer instead
      // would draw a mixed run in whichever namespace one of them happened to
      // be in, and a box in a colour none of them had.
      const agreed = (of) => {
        const first = of(members[0]);
        return members.every((m) => of(m) === first) ? first : undefined;
      };
      const box = {
        id: f.stands,
        type: agreed((m) => m.type) || f.kind,
        name: f.label || `${members[0].name} ×${members.length}`,
        provider: agreed((m) => m.provider),
        attrs: {fold: f.kind, members: members.length},
      };
      // Coverage is the subject of a whole class of these drawings, and no
      // coverage means nobody knows — so dropping it turns a box everybody
      // has looked at into one nobody has.
      if (agreed((m) => (m.coverage ? m.coverage.state : ''))) box.coverage = members[0].coverage;
      // The claim is assembled field by field, the way it is in Go. Comparing
      // only the origin and the author would let a box show one member's note
      // — or one member's confidence — as though it had been said about all of
      // them, and a reader has no way to tell that it was not.
      box.claim = agreedClaim(members);
      const groups = {};
      for (const axisID of Object.keys(members[0].groups || {})) {
        const path = agreed((m) => (m.groups || {})[axisID]);
        if (path) groups[axisID] = path;
      }
      if (Object.keys(groups).length) box.groups = groups;

      // What happens inside the fold. A line from the box to itself says
      // nothing and draws badly, so it is a number here as it is there.
      const held = new Set(members.map((m) => m.id));
      let internal = 0;
      for (const e of graph.edges) {
        if (!e.suppressed && held.has(e.from) && held.has(e.to)) internal++;
      }
      if (internal > 0) box.attrs.internal_references = internal;
  ...
  // field the members disagree about is left off, which is what an absent
  // optional field means here anyway. An unstated confidence is not a
  // confidence of zero, so the two are told apart.
  function agreedClaim(members) {
    const claims = members.map((m) => m.claim || null);
    if (!claims.some(Boolean)) return undefined;

    const origin = (c) => (c ? c.origin : 'parser');
    if (!claims.every((c) => origin(c) === origin(claims[0]))) return undefined;

    const out = {origin: origin(claims[0])};
    const agreesOn = (of) => {
      const first = of(claims[0]);
      return claims.every((c) => of(c) === first) ? {value: first} : null;
    };
    const author = agreesOn((c) => (c && c.author) || '');
    if (author && author.value) out.author = author.value;
    const note = agreesOn((c) => (c && c.note) || '');
    if (note && note.value) out.note = note.value;
    const confidence = agreesOn((c) =>
      c && c.confidence !== undefined ? String(c.confidence) : 'unstated');
    if (confidence && claims[0] && claims[0].confidence !== undefined) {
  ...
  // How many members a box lists before it says how many more there are.
  const MEMBERS = 8;
  ...
  function nodeLabels(n) {
    ...
    // What kind of type it is. A class diagram that draws an interface and a
    // struct the same way has lost the thing it was drawn for, and the two are
    // one word apart in the document. Only for a type: `kind` means something
    // else on a Kubernetes object, and the box there already says it.
    if (n.type === 'code_type' && n.attrs && typeof n.attrs.kind === 'string' && n.attrs.kind) {
      second = n.attrs.kind;
    }
  ...
  // What a type declares, listed inside its box.
  //
  // This is UML's own answer and it is the right one: a class with nine
  // methods drawn as nine boxes is a picture of nine things, when it is a
  // picture of one thing with nine methods. Nothing is lost — the file that
  ...
  function declaredBy(n) {
    ...
    // A type with forty methods is a box the length of the page. The count
    // says what was left out, so the box is honest about being a summary.
    if (members.length > MEMBERS) shown.push(`… 他 ${members.length - MEMBERS} 件`);
    return shown;
  }
  ...
  function nodeSize(n) {
    ...
    // Every box is the same height until a document gives it members. That
    // is not "the height follows the text" coming back: a longer name still
    // changes nothing, because a two-line name is not a bigger thing than a
    // one-line one. A list of members is a different claim — it is what the
    // type declares — and a box that does not grow for it draws them outside
    // itself.
    const members = declaredBy(n);
    const extra = members.length ? members.length * LINE + 8 : 0;
    return {
      width: sized.width || Math.max(104, Math.ceil(w) + PAD_X * 2 + ICON + ICON_GAP + door),
      // A size given by hand can make a box bigger but not smaller than what
      // it has to hold. A layout document is a file somebody edits, and a
      // height saved before the type had members — or simply typed in — would
      // otherwise draw the member list and the compartment rule outside the
      // box they belong to.
      height: Math.max(sized.height || 0, BOX_HEIGHT + extra),
    };
  }
  ...
  function sequenceLayout() {
    ...
    for (const e of allEdges().filter((e) => e.attrs && e.attrs.step).sort((a, b) => a.attrs.step - b.attrs.step)) {
      // Counted per participant rather than per step: both ends of the first
      // message share a step number, and two columns that tie fall back to
      // the order the document happens to list them in — which can put the
      // one being called to the left of the one calling it, and draw the
      // first message of the sequence right to left.
      for (const id of [e.from, e.to]) {
        if (!first.has(id)) first.set(id, appearance++);
      }
    }
    order.sort((a, b) => {
  ...
  function buildRoot() {
    ...
    //
    // Twenty lines that land on the same pair are one line carrying how many
    // it stands for. Without that the page draws twenty where the SVG of the
    // same graph draws one — the folded picture, rendered as a hairball —
    // because folding here happens after the document was normalized and
    // nothing else would merge them.
    const edges = [];
    ...
    for (const [i, e] of allEdges().entries()) {
      ...
      const drawnEdge = {
        id: 'e' + i, sources: [from], targets: [to],
        infra: {
  ...
  function selectEdge(key) {
    ...
    // A disagreement is about the edge, not about which of its steps was
    // clicked, and the document names it the way the IR does.
    const conflicts = edgeConflicts.filter((c) => c.target === claimedKey(key));
    if (conflicts.length) {
      const s = section('disagreement');
      for (const c of conflicts) {
        const ul = document.createElement('ul');
